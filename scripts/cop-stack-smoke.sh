...
SEMOPS_COP_SMOKE_WEATHER_ENABLED="$WEATHER_SMOKE_ENABLED" \
  go test ./internal/smoke/cop -run 'TestHostedCOP(SnapshotReflects(MAVLinkUDP|CoTUDP|ScenarioRunner|ADSBHTTPProvider|HADRSharedAirspace|KLVLocalMedia|WeatherFixture)|ComponentPrometheusMetricsReflectFeedFlow|RuntimeReflectsFeedFlow)' -count=1 -v
