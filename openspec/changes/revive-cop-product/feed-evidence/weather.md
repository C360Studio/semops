...
- `internal/projectors/weather` now contains a mutation-plan projector and graph writer for weather observations. It
  proves the graph contract, owner-token fence, create/update request boundary, and typed mutation failure path without
  hosted runtime wiring, UI, or route-safety rules.
