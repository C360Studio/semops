...
// ConvertToSemStreamsMessage converts a MAVLink packet to SemStreams message
func (p *MAVLinkParser) ConvertToSemStreamsMessage(packet *MAVLinkPacket) (message.Message, error) {
	switch packet.MessageID {
	case constants.MavlinkMsgIdHeartbeat:
		return p.convertHeartbeat(packet)
	case constants.MavlinkMsgIdGlobalPositionInt:
		return p.convertGlobalPosition(packet)
	case constants.MavlinkMsgIdAttitude:
		return p.convertAttitude(packet)
	case constants.MavlinkMsgIdBatteryStatus:
		return p.convertBattery(packet)
	default:
		return nil, fmt.Errorf("unsupported message ID: %d", packet.MessageID)
	}
}

// convertHeartbeat converts MAVLink heartbeat to SemStreams heartbeat
func (p *MAVLinkParser) convertHeartbeat(packet *MAVLinkPacket) (message.Message, error) {
	if packet.ParsedFields == nil {
		return nil, fmt.Errorf("no parsed fields available")
	}

	heartbeat := payloads.NewHeartbeatPayload(packet.SystemID, packet.ComponentID, time.Now())
	heartbeat.Ts = packet.Timestamp

	if vehicleType, ok := packet.ParsedFields["type"].(uint8); ok {
		heartbeat.VehicleType = vehicleType
	}

	if autopilot, ok := packet.ParsedFields["autopilot"].(uint8); ok {
		heartbeat.Autopilot = autopilot
	}

	if baseMode, ok := packet.ParsedFields["base_mode"].(uint8); ok {
		heartbeat.BaseMode = baseMode
	}

	if customMode, ok := packet.ParsedFields["custom_mode"].(uint32); ok {
		heartbeat.CustomMode = customMode
	}

	if systemStatus, ok := packet.ParsedFields["system_status"].(uint8); ok {
		heartbeat.SystemStatus = systemStatus
	}

	if mavlinkVersion, ok := packet.ParsedFields["mavlink_version"].(uint8); ok {
		heartbeat.MavlinkVersion = mavlinkVersion
	}

	// Wrap payload in BaseMessage
...
// convertGlobalPosition converts MAVLink global position to SemStreams position
func (p *MAVLinkParser) convertGlobalPosition(packet *MAVLinkPacket) (message.Message, error) {
	if packet.ParsedFields == nil {
		return nil, fmt.Errorf("no parsed fields available")
	}

	lat, ok1 := packet.ParsedFields["lat"].(int32)
	lon, ok2 := packet.ParsedFields["lon"].(int32)
	if !ok1 || !ok2 {
		return nil, fmt.Errorf("missing lat/lon fields")
	}

	latDeg := float64(lat) / 1e7
	lonDeg := float64(lon) / 1e7

	position := payloads.NewPositionPayload(packet.SystemID, time.Now(), latDeg, lonDeg)
	position.Ts = packet.Timestamp
	...
	if hdg, ok := packet.ParsedFields["hdg"].(uint16); ok {
		position.Hdg = hdg
	}
...
// convertAttitude converts MAVLink attitude to SemStreams attitude
func (p *MAVLinkParser) convertAttitude(packet *MAVLinkPacket) (message.Message, error) {
	if packet.ParsedFields == nil {
		return nil, fmt.Errorf("no parsed fields available")
	}

	attitude := payloads.NewAttitudePayload(packet.SystemID, time.Now())
	attitude.Ts = packet.Timestamp
	...
	if rollSpeed, ok := packet.ParsedFields["rollspeed"].(float32); ok {
		attitude.RollSpeed = rollSpeed
	}

	if pitchSpeed, ok := packet.ParsedFields["pitchspeed"].(float32); ok {
		attitude.PitchSpeed = pitchSpeed
	}

	if yawSpeed, ok := packet.ParsedFields["yawspeed"].(float32); ok {
...
// convertBattery converts MAVLink battery status to SemStreams battery
func (p *MAVLinkParser) convertBattery(packet *MAVLinkPacket) (message.Message, error) {
	if packet.ParsedFields == nil {
		return nil, fmt.Errorf("no parsed fields available")
	}

	batteryID := uint8(0)
	if id, ok := packet.ParsedFields["id"].(uint8); ok {
		batteryID = id
	}

	battery := payloads.NewBatteryPayload(packet.SystemID, batteryID, time.Now())
	battery.Ts = packet.Timestamp

	if remaining, ok := packet.ParsedFields["battery_remaining"].(int8); ok {
		battery.BatteryRemaining = remaining
	}

	if current, ok := packet.ParsedFields["current_battery"].(int16); ok {
		battery.CurrentBattery = current
	}

	if consumed, ok := packet.ParsedFields["current_consumed"].(int32); ok {
		battery.CurrentConsumed = consumed
	}

	if energy, ok := packet.ParsedFields["energy_consumed"].(int32); ok {
		battery.EnergyConsumed = energy
	}

	if temp, ok := packet.ParsedFields["temperature"].(int16); ok {
		battery.Temperature = temp
	}

	if voltages, ok := packet.ParsedFields["voltages"].([]uint16); ok {
		battery.Voltages = voltages
	}

	if chargeState, ok := packet.ParsedFields["charge_state"].(uint8); ok {
		battery.ChargeState = chargeState
	}
