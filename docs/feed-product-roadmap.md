...
nullable position fields plus position-source quality before projection. The next implemented slice projects
source-partitioned ADS-B aircraft tracks with `signal` indexing and reads them back through COP graph prefix
discovery.
...
Not claimed yet:
Hosted ADS-B adapter behavior, deterministic replay runner, live air-traffic feed reliability, ASTERIX support,
cross-source aircraft association, or complete surveillance/radar processing.
