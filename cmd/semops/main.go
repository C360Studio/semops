	...
	if runtime != nil {
		if requester := runtime.GraphRequester(); requester != nil {
			graphProvider, err := copapi.NewGraphProvider(
				...
				copapi.WithGraphDiscovery(cfg.COP.GraphDiscoveryEnabled),
				copapi.WithGraphDiscoveryLimit(cfg.COP.GraphDiscoveryLimit),
				copapi.WithGraphDiscoveryScopes([]copapi.GraphDiscoveryScope{
					{Org: cfg.MAVLink.Org, Platform: cfg.MAVLink.Platform},
					{Org: cfg.CoT.Org, Platform: cfg.CoT.Platform},
				}),
