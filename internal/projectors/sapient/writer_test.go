...
func TestGraphWriterAppliesSAPIENTCreateAndUpdate(t *testing.T) {
	...
	if create.Entity.ID != "sapient-track-1" {
	...
	if update.Entity.ID != "sapient-track-1" {
...
func TestGraphWriterReturnsSAPIENTMutationFailure(t *testing.T) {
	...
	if mutationErr.Kind != MutationCreate ||
		mutationErr.EntityID != "sapient-track-1" ||
...
func TestGraphWriterHonorsSAPIENTCustomSubjects(t *testing.T) {
	...
	writer := NewGraphWriter(
		requester,
		WithMutationSubjects("sapient.graph.create", "sapient.graph.update"),
	)
	err := writer.Apply(context.Background(), Plan{Mutations: []Mutation{{
		Kind: MutationCreate,
		Create: graph.CreateEntityWithTriplesRequest{
	...
	if len(requester.calls) != 1 || requester.calls[0].subject != "sapient.graph.create" {
