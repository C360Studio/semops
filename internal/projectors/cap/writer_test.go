...
func TestGraphWriterAppliesCAPCreateAndUpdate(t *testing.T) {
	...
	plan := Plan{Mutations: []Mutation{
		{
			...
			Create: graph.CreateEntityWithTriplesRequest{
				Entity: &graph.EntityState{ID: "hazard-1"},
			},
		},
		{
			Kind: MutationUpdate,
	...
	if update.Entity.ID != "hazard-1" {
		t.Fatalf("update entity id = %q", update.Entity.ID)
	}
}

func TestGraphWriterReturnsMutationFailure(t *testing.T) {
	requester := &recordingRequester{
		errors: []error{classifiedEntityExistsError("hazard-1")},
	}
	...
	if mutationErr.Kind != MutationCreate || mutationErr.EntityID != "hazard-1" ||
