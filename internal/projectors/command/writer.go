...
func WithProjector(projector *Projector) GraphWriterOption {
	return func(writer *GraphWriter) {
		writer.projector = projector
	}
}

func (w *GraphWriter) Apply(ctx context.Context, plan Plan) error {
	if w == nil || w.requester == nil {
		return fmt.Errorf("command graph writer has no requester")
	}
	for i, mutation := range plan.Mutations {
		if err := ctx.Err(); err != nil {
			return fmt.Errorf("apply command mutation %d: %w", i, err)
		}
	...
	if w.projector != nil {
		w.projector.MarkBornForPlan(plan)
	}
	return nil
}
