...
func TestGraphWriterAppliesADSBCreateAndUpdate(t *testing.T) {
	...
	plan := Plan{Mutations: []Mutation{
		{
			...
			Create: graph.CreateEntityWithTriplesRequest{
				Entity: &graph.EntityState{ID: "track-1"},
			},
		},
		{
			Kind: MutationUpdate,
...
func TestGraphWriterReturnsMutationFailure(t *testing.T) {
	requester := &recordingRequester{
		errors: []error{classifiedEntityExistsError("track-1")},
	}
	...
	if mutationErr.Kind != MutationCreate || mutationErr.EntityID != "track-1" ||
